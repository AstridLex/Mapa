...
def crear_mapa(filas, columnas):
    mapa = np.full((filas, columnas), " ", dtype=str)
    return mapa

def poner_obstaculos(mapa, obstaculos):
...
poner_obstaculos(mapa, obstaculos)

def obtener_coordenadas(mensaje, filas, columnas):
    while True:
...
inicio = obtener_coordenadas("Coloque las coordenas para el punto de inicio separadas por espacios. Ingrese desde el 0 al 30 (x y): ", filas, columnas)
final = obtener_coordenadas("Coloque las coordenas para el punto de llegada separadas por espacios. Ingrese desde el 0 al 30 (x y): ", filas, columnas)

mapa[inicio[0]][inicio[1]] = 'I'
mapa[final[0]][final[1]] = 'F'
...
def a_estrella(mapa, inicio, final):
    ...
    cerrada = {} 
    g_puntaje = {inicio: 0}
    f_puntaje = {inicio: heuristica(inicio, final)}
    ...
    while abierta:
        ...
        for vecino in vecinos:
            if 0 <= vecino[0] < filas and 0 <= vecino[1] < columnas and mapa[vecino[0]][vecino[1]] != '#':
                ...
                if vecino not in g_puntaje or posible_g < g_puntaje[vecino]:
                    ...
                    f_puntaje[vecino] = posible_g + heuristica(vecino, final)
...
def agregar_obstaculos(filas, columnas):
    obstaculos = []
    while True:
        entrada = input("En caso de que tenga un obstaculo que no aparezca en la pantalla, favor ingrese la ubicacion con coordenadas del 0 al 30 (x y) o 'X' poara continuar sin agregar: ")
        if entrada.lower() == 'x':
            break
        if ' ' in entrada:
            x, y = entrada.split()
            if x.isdigit() and y.isdigit():
                ...
                if 0 <= x < filas and 0 <= y < columnas:
                    ...
                    print(f"Las coordenadas ingresadas no son validas")
            else:
                print("444")
    return obstaculos

nuevos_obstaculos = agregar_obstaculos(filas, columnas)
poner_obstaculos(mapa, nuevos_obstaculos)
